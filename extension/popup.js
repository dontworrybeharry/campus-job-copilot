...
chrome.storage.local.get({profile:null,syncedAt:"",jobs:[],resume:null},s=>{
  $("status").innerHTML=s.profile
    ...
    :`<span class="warn">还没有同步档案</span>：打开秋招作战台 →「AI 助手」→「同步档案到插件」。`;
});
$("grab").onclick=async()=>{
...
$("saveJob").onclick=()=>{
  ...
  chrome.storage.local.get({jobs:[]},r=>chrome.storage.local.set({jobs:[...r.jobs,job]},()=>{ $("job").hidden=true; $("jobMsg").innerHTML='<span class="ok">已加入</span>：打开秋招作战台时会自动导入投递看板。'; }));
};
$("fill").onclick=async()=>{
...
$("open").onclick=()=>chrome.storage.local.get({platformUrl:""},r=>{ if(r.platformUrl) chrome.tabs.create({url:r.platformUrl}); else $("fillMsg").textContent="先在浏览器里打开一次秋招作战台。"; });
